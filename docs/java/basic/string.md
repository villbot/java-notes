...
compareTo() 方法会循环对比所有的字符，当两个字符串中有任意一个字符不相同时，则 return char1-char2。

比如，两个字符串分别存储的是 1 和 2，返回的值是 -1；如果存储的是 1 和 1，则返回的值是 0 ，如果存储的是 2 和 1，则返回的值是 1。
...
1. **== 对于基本数据类型来说，是用于比较 “值”是否相等的；**

2. **而对于引用类型来说，是用于比较引用地址是否相同的。**
...
总知，使用 final 修饰的第一个好处是**安全**；第二个好处是**高效**，以 JVM 中的字符串常量池来举例，如下两个变量：  
...
<img src="https://elgchat-oss.oss-cn-zhangjiakou.aliyuncs.com/typora/image-20200807171052188.png" alt="image-20200807171052188" style="zoom:50%;" />
...
<img src="https://elgchat-oss.oss-cn-zhangjiakou.aliyuncs.com/typora/image-20200807171250359.png" alt="image-20200807171250359" style="zoom:50%;" />
